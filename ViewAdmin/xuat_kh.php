...
if (is_array($hocki) || is_object($hocki))
{
	...
	while ($row = $data->fetch_assoc() ) {
		...
		$tongsotc += $row['soTc'];
		$tongsotiet += ($row['soTietLt']+$row['soTietTh']+$row['soTietKt']);
		$tongsotiet_lt += $row['soTietLt'];
		$tongsotiet_bt += $row['soTietTh'];
		$tongsotiet_kt += $row['soTietKt'];
		$rows++;
	    
	}
	$ds[] = array('STT' => '','mahocphan'=>'','tenmon'=>'Tổng cộng','sotc' =>$tongsotc,'batbuoc'=>'','tuchon'=>'','tongsotiet'=>$tongsotiet,'sotietlt'=>$tongsotiet_lt,'sot_th' => $tongsotiet_bt,'kt'=> $tongsotiet_kt);
